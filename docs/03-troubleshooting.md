...
## 3x-ui: пустой порт/логин и «Database initialization failed»

**Симптом**: в выводе установки видно `Database initialization failed: XUI_DB_TYPE=postgres but XUI_DB_DSN is empty`, а `Access URL` без порта (`http://IP:/path`), логин/пароль не показаны. Панель фактически не настроена.
...
## bootstrap.sh падает с «ХЕШИ НЕ СОВПАДАЮТ»

**Симптом**: `bootstrap.sh` останавливается с `ХЕШИ НЕ СОВПАДАЮТ` при попытке запустить `deploy.sh` на новом сервере.

**Причина**: `deploy/deploy.sh.sha256` хранит SHA256 от `deploy/deploy.sh` на момент последнего коммита хеша. Если `deploy.sh` поправили, а хеш не пересчитали — они расходятся, и это ожидаемая защита от подмены/повреждения файла.

**Лечение**: после **любой** правки `deploy/deploy.sh` пересчитать хеш и закоммитить вместе с изменением:
```bash
sha256sum deploy/deploy.sh | awk '{print $1}' > deploy/deploy.sh.sha256
git add deploy/deploy.sh deploy/deploy.sh.sha256
git commit -m "..."
```
